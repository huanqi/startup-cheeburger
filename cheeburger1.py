...
data['Service'] = data['Service'].map( {'Excellent':2, 'Good':1, 'Fair':0, 'Poor':-1, 'Terrible':-2}).astype(int)


# convert timely to 1/0
Time = data['Timely'].values



# convert worthy to 1/0
Worth = data['Worthy'].values
...
#feedback_index = data['Service'].values + 4* Time + 4* Worth + data['Cleanliness'].values
#print 'feadback_index', feedback_index
...
#output = pd.DataFrame( data = {"recommendation":data['Refer'].values, "index":feedback_index} )
#output.to_csv( "refervsindex.csv", index = False, quoting = 2)

# output all the relevant variables
output = pd.DataFrame( data = {"service":data['Service'].values, "timely":Time, "worthy":Worth, "cleanliness":data['Cleanliness'].values, "recommendation":data['Refer'].values })

output.to_csv("parameters.csv", index = False, quoting = 2)
